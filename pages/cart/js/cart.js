...
function getAllCart() {
  cartRow.innerHTML = user.cart.map(item => `
...
const deleteCard = document.querySelectorAll(".trending__card-btn");

Array.from(deleteCard).forEach(item => {
  item.addEventListener("click", (e) => {
    ...
    if (user) {
      fetch(`http://localhost:3000/user/${user.id}`, {
        ...
        body: JSON.stringify({
          cart: user.cart.filter(i => i.id != item.dataset.id)
        })
      })
        .then(res => res.json())
        .then(res => {
